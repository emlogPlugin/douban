...
/*************以下为自定义******************/		
		$this->expiresIn = $result->expires_in;//已添加
		$this->doubanUserid = $result->douban_user_id;//已添加
/*************以上为自定义******************/
	}
...
/*************以下为自定义******************/		
    ...
    public function refresh()
    {
        ...
        $result = json_decode($this->curl($refreshUrl, 'POST', $header, $data));
        $this->accessToken = $result->access_token;
        $this->refreshToken = $result->refresh_token;		
		$this->expiresIn = $result->expires_in;//已添加
		$this->doubanUserid = $result->douban_user_id;//已添加
...
/*************以下为自定义******************/
    ...
    /**
     * @brief 获取doubanUserid
     *
     * @return string
     */
    public function getDoubanUserid()
    {
...
/*************以上为自定义******************/
     ...
     */
/*************以下为自定义******************/
    public function getAuthorizeUrl() //protected属性修改为public
/*************以上为自定义******************/
    {
        $params = array(
                    'client_id' => $this->clientId,
